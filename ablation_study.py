...
class RadGenAblation(RadGen):
    ...
    def forward(self, images, clinical_ids=None, clinical_mask=None,
        ...
        # Call parent forward
        outputs = super().forward(images, clinical_ids, clinical_mask,
                                 report_ids, report_mask, labels, mode)
...
def run_ablation_experiment(config_name, model_config, train_loader, val_loader, device, num_epochs=30):
    ...
    criterion = CombinedLoss(report_weight=0.3 if model_config.get('use_report_gen', True) else 0.0)
