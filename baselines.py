...
class ReportOnlyBaseline(nn.Module):
    ...
    def __init__(self, num_classes=3):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(768, 256), nn.GELU(), nn.Dropout(0.3), nn.Linear(256, num_classes)
        )
...
class EarlyFusionBaseline(nn.Module):
    ...
    def __init__(self, num_classes=3):
        ...
        self.fusion = nn.Sequential(
            nn.Linear(1024 + 768, 512), nn.LayerNorm(512), nn.GELU(), nn.Dropout(0.3)
        )

        self.classifier = nn.Sequential(
            nn.Linear(512, 256), nn.GELU(), nn.Dropout(0.3), nn.Linear(256, num_classes)
        )
...
def train_baseline(
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            if isinstance(model, DenseNet121Baseline) or isinstance(
                model, SwinBaseline
            ):
        ...
        logger.info(
            f"{model_name} Epoch {epoch + 1}: Val AUC={val_metrics['macro_auc']:.4f}, "
...
def run_all_baselines(fold=4, batch_size=8):
    ...
    results["densenet121"] = train_baseline(
        model, train_loader, val_loader, device, model_name="densenet121"
    )
    ...
    results["swin_b"] = train_baseline(
        model, train_loader, val_loader, device, model_name="swin_b"
    )
    ...
    results["report_only"] = train_baseline(
        model, train_loader, val_loader, device, model_name="report_only"
    )
    ...
    results["early_fusion"] = train_baseline(
        model, train_loader, val_loader, device, model_name="early_fusion"
    )
    ...
    results["late_fusion"] = train_baseline(
        model, train_loader, val_loader, device, model_name="late_fusion"
    )
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(
        f"{'Method':<20} {'Macro AUC':>10} {'Accuracy':>10} {'Normal AUC':>12} {'Pneumonia AUC':>14} {'TB AUC':>10}"
    )
    print("-" * 80)
    for name, metrics in results.items():
        print(
            f"{name:<20} {metrics['macro_auc']:>10.4f} {metrics['accuracy']:>10.4f} "
