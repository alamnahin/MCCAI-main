...
def main():
    ...
    try:
        train_ds = RadGenDataset(
            ...
            fold=Config.TARGET_FOLD, split='train',
            transform=get_transforms('train')
        )
        
        val_ds = RadGenDataset(
            Config.CSV_PATH, Config.IMG_DIR,
            fold=Config.TARGET_FOLD, split='val',
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        # Logging
        logger.info(f"Train Loss: {train_loss:.4f} (CLS: {cls_loss:.4f}, REP: {rep_loss:.4f}) | "
                   f"Train Acc: {train_acc:.4f}")
        logger.info(f"Val AUC: {val_metrics['macro_auc']:.4f} | "
                   f"Val Acc: {val_metrics['accuracy']:.4f} | "
                   f"Val F1: {val_metrics['macro_f1']:.4f}")
        
        # Per-class metrics
        for cls in ['Normal', 'Pneumonia', 'TB']:
            logger.info(f"  {cls:10s} - AUC: {val_metrics[f'{cls.lower()}_auc']:.4f}, "
    ...
    final_metrics, final_probs, final_labels = validate(model, val_loader, Config.DEVICE)
